...
int a[10000001];
int n;
long long int minimum(int node,int l_n,int h_n,int l_r,int h_r)
{
    if( l_r<=l_n && h_n<=h_r )
    return tree[node];
    if( h_n<l_r || h_r<l_n)
    ...
    return min(minimum(2*node+1,l_n,mid,l_r,h_r),minimum(2*node+2,mid+1,h_n,l_r,h_r));
}
